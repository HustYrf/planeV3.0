...
import hust.plane.utils.FileMD5Util;
...
//        stringRedisTemplate.delete(WebConst.FILE_UPLOAD_STATUS);
//        stringRedisTemplate.delete(WebConst.FILE_MD5_KEY);
...
//    }
    ...
    private boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
        ...
        if (isComplete == Byte.MAX_VALUE) {
            stringRedisTemplate.opsForHash().put(WebConst.FILE_UPLOAD_STATUS, param.getMd5(), "true");
            stringRedisTemplate.opsForValue().set(WebConst.FILE_MD5_KEY + param.getMd5(), uploadDirPath + "/" + fileName);
            return true;
        } else {
            if (!stringRedisTemplate.opsForHash().hasKey(WebConst.FILE_UPLOAD_STATUS, param.getMd5())) {
                stringRedisTemplate.opsForHash().put(WebConst.FILE_UPLOAD_STATUS, param.getMd5(), "false");
            }
            if (stringRedisTemplate.hasKey(WebConst.FILE_MD5_KEY + param.getMd5())) {
                stringRedisTemplate.opsForValue().set(WebConst.FILE_MD5_KEY + param.getMd5(), uploadDirPath + "/" + fileName + ".conf");
            }
            return false;
        }
    }
