...
(function(global, factory){
    ...
    hivideo.prototype = {
        ...
        init: function(ele){
            ...
            <div class="video-reflush float"> \
            <button hivideoId="flushBtn" class="flushBtn"></button> \
            </div> \
            <div class="video-fullscreen right"> \
            <button hivideoId="fullBtn" class="onBtn"></button> \
            </div>\
            <div class="video-volume right"> \
            <input type="range" hivideoId="volumebar" class="volumebar" value="10" min="0" max="100" step="0.1"> \
            </div> \
            <div class="video-mute right"> \
            ...
            this.flushBtn = hivideo.querySelector('button[hivideoId="flushBtn"]', videoWrapper);
            ...
            this.bindFullEvent().bindPlayEvent().bindMuteEvent().bindFlushEvent().bindAutoHideControlsEvent();
            this.config();
    ...
    hivideo.prototype.bindFlushEvent=function(){
         var self = this;
       
        [self.flushBtn, self.videoPlayIcon].forEach(function(ele){
            ele.addEventListener("click", function(event){
                event = event || window.event;
                if(!self.video) return;    
                //在此刷新视频播放 
                if(!self.video.isNext){
                   self.video.isNext = true; 
                }else{
                    
                    //这里可以用ajax获取新的视频源
                    document.getElementById("videosrc").setAttribute("src","res/assets/video.mp4");
                    //self.video.src("assets/video2.mp4");                 
                     self.video.load(); 
                    //重新加载播放器            
